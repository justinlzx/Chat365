...
def get_model_response(query: str, db: Chroma = None) -> Generator[str, None, None]:
    try:
        ...
        system_prompt = """
You are a helpful consulting assistant whose job is to assist the user with answering queries only to business and financial information. If the user asks a question that is not related, politely remind them that you can only answer business-related questions.
